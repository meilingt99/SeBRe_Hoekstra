...
def build_fpn_mask_graph(rois, feature_maps,
    ...
    x = KL.TimeDistributed(KL.Conv2DTranspose(256, (2, 2), strides=2, activation="relu"),
                           name="mrcnn_mask_deconv1")(x) # [MEILING] renamed by adding a 1 when adding a conv2dtranspose
    x = KL.TimeDistributed(KL.Conv2DTranspose(256, (2, 2), strides=2, activation="relu"),
                           name="mrcnn_mask_deconv2")(x) # [MEILING] added when changing mask size from 28x28 --> 56x56
    # x = KL.TimeDistributed(KL.Conv2DTranspose(256, (2, 2), strides=2, activation="relu"),
    #                    name="mrcnn_mask_deconv3")(x) # [MEILING] added when changing mask size from 56x56 --> 112x112
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    print('loaded image_id ',image_id)
...
def data_generator(dataset, config, shuffle=True, augment=False, random_rois=0,
    ...
    while True:
        try:
            ...
            print("(in data generator) number of images: ",len(image_ids))
...
class MaskRCNN():
    ...
    def set_log_dir(self, model_path=None):
        ...
        print("self.checkpoint_path: ",self.log_dir)
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        # Callbacks
        print("CSV DIR: ",os.path.join(self.model_dir,'epoch_log.csv'))
        callbacks = [
            keras.callbacks.TensorBoard(log_dir=self.log_dir,
                                        histogram_freq=0, write_graph=True, write_images=False),
            keras.callbacks.ModelCheckpoint(self.checkpoint_path,
                                            verbose=0, save_weights_only=True),
            keras.callbacks.CSVLogger(os.path.join(self.model_dir,'epoch_log.csv'), append=True, separator=','),
        ]
