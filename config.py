...
class Config(object):
    ...
    # implementation.
    LEARNING_RATE = 0.001
    LEARNING_MOMENTUM = 0.9 #changed for training 64px
...
class BrainConfig(Config):
    ...
    # Reduce training ROI=s per image because the images are small and have
    # few objects. Aim to allow ROI sampling to pick 33% positive ROIs.
    TRAIN_ROIS_PER_IMAGE = 32

    # Use a small epoch since the data is simple
    STEPS_PER_EPOCH = 2000 #2000 #steps_per_epoch: Total number of steps (batches of samples) before declaring one epoch finished and starting the next epoch. 
    ...
    VALIDATION_STEPS = 19 #100 #validation_steps = TotalvalidationSamples / ValidationBatchSize
    ...
    ###### Further changes (experimentation):
     ...
     # Maximum number of ground truth instances to use in one image
    MAX_GT_INSTANCES = 1 #8 #100 #decreased to avoid duplicate instances of each brain region
    
    # Max number of final detections
    DETECTION_MAX_INSTANCES = 1 #8 #100 # #decreased to avoid duplicate instances of each brain region

    # Minimum probability value to accept a detected instance
    # ROIs below this threshold are skipped
    DETECTION_MIN_CONFIDENCE =  0.75 #0.7
